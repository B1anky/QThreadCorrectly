...
void WorkerNetwork::createNewWorker(){
    ...
    randomizerWorker->setup(sorterThread, QString::number(m_workers));
    randomizerWorker->moveToThread(sorterThread);

    setupThreadAndWorkerConnection(generateThread, sortedWorker, sortButton);
...
void WorkerNetwork::setupThreadAndWorkerConnection(QThread* thread, Worker* worker, QPushButton* button){
    connect(button, &QPushButton::clicked, this, [this, thread, worker](){
        if(m_threadedCheckBox->isChecked() && !thread->isRunning()){
            thread->start();
        }else{
